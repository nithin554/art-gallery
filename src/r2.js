...
/**
 * The Worker URL that returns the AI overview for an artwork's key, i.e.
 * JSON { name, description } generated by Gemini vision from the image.
 */
export function describeUrl(key) {
  return `${R2_CONFIG.imageBaseUrl.replace(/\/+$/, '')}/describe/${String(
    key
  ).replace(/^\/+/, '')}`;
}
